...
weather_forecast=pd.read_csv(r'C:\Users\afedgo\Desktop\Practice\Datathon 2021\weather_forecast.csv',parse_dates=['date'])
...
""" Weather forecast data set:  Map cities to region, add time to date, average out all weather variables by date and time"""


weather_forecast["region"]=np.select(condlistCities3,choicelist)
...
weather_fore_grouped.set_index(['date','region'])

template=pd.DataFrame(pd.to_datetime(pd.period_range("6/12/2021", freq="H", periods=193).to_timestamp(),utc=True))
...
template_all=pd.concat([template, template2, template3, template4, template5, template6, template7, template8])


weather_forecast2=pd.merge(
    template_all,
...
weather_forecast2.to_csv(r'C:\Users\afedgo\Desktop\Practice\Datathon 2021\Weather Test.csv') 
